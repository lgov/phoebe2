...
def _compare_orbit_bs(b, verbose=False):
    """
    """
    ...
    ts, xs, ys, zs, vxs, vys, vzs, inst_ds, inst_Fs, inst_ethetas, inst_elongans, inst_eincls, inst_periods, inst_smas, inst_eccs, inst_per0s, inst_long_ans, inst_incls, inst_t0_perpasses = phoebe.dynamics.nbody.dynamics_from_bundle_bs(b, times=[0], return_roche_euler=True)

    sma = inst_smas[0][0]
    ecc = inst_eccs[0][0]
    period = inst_periods[0][0]
    incl = inst_incls[0][0]
    long_an = inst_long_ans[0][0]
    per0 = inst_per0s[0][0]
    t0_perpass = inst_t0_perpasses[0][0]

    orbit_ps = b.filter(component='binary', context='component')
            ...
            for p in [3]:
                ...
                for per0 in [0, 15]:
                    ...
                    for incl in [90, 45, 0]:
                        ...
                        _compare_orbit_bs(b, verbose=verbose)
